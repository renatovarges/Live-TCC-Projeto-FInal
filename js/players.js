...
const CLUB_MAPPING = {
  ...
  'Vasco': 'vasco',
  'Vitória': 'vitoria',
  // Códigos de 3 letras do CSV e API
  'CAM': 'atletico-mg',
  'CAP': 'athletico-pr',
  'BAH': 'bahia',
  'BOT': 'botafogo',
...
function processPlayersData(rawData) {
  const processed = rawData.map(player => {
    // Normalizar nome do clube
    const clubName = player.time || player.clube || player['time'] || '';
    const normalizedClub = CLUB_MAPPING[clubName] || clubName.toLowerCase().replace(/\s+/g, '-');
    ...
    return {
      ...
      originalClub: player.time || player.clube || player['time'] || '', // Manter o código original do CSV
...
async function autoLoadData() {
  ...
  updateInterval = setInterval(async () => {
    ...
    await loadData();
  }, 5 * 60 * 1000); // 5 minutos para capturar mudanças da API
}
...
async function loadData() {
  ...
  try {
    console.log('🔄 Carregando dados da API do Cartola...');
    ...
    if (apiData && apiData.atletas) {
      apiPlayers = processCartolaData(apiData);
      console.log(`✅ ${apiPlayers.length} jogadores carregados da API`);
    }
    
    // Carregar dados do CSV como complemento
    console.log('🔄 Carregando dados complementares do CSV...');
    const response = await fetch('./cartola_jogadores_time_posicao_preco.csv');
    const text = await response.text();
    const rawData = parseCSV(text);
    let csvPlayers = processPlayersData(rawData);
    console.log(`✅ ${csvPlayers.length} jogadores carregados do CSV`);
    
    // Fazer merge dos dados: API tem prioridade, CSV complementa
    STATE.players = mergePlayerData(apiPlayers, csvPlayers);
    ...
    window.dispatchEvent(new CustomEvent('playersLoaded', { detail: STATE.players }));
    
  } catch (error) {
    console.error('❌ Erro ao carregar dados:', error);
    
    // Fallback para CSV apenas
    try {
      const response = await fetch('./cartola_jogadores_time_posicao_preco.csv');
      const text = await response.text();
      const rawData = parseCSV(text);
      STATE.players = processPlayersData(rawData);
      lastUpdateTime = new Date();
      ...
      console.log(`✅ Fallback: ${STATE.players.length} jogadores do CSV`);
      window.dispatchEvent(new CustomEvent('playersLoaded', { detail: STATE.players }));
    } catch (csvError) {
      console.error('❌ Erro no fallback CSV:', csvError);
    }
  }
}
